...
function buildEmailHtml(data) {
  ...
  const items = rows.map(function ([label, value]) {
    return '<tr><td style="padding:8px 12px;border-bottom:1px solid #eee;font-weight:bold;color:#0b5cd8;width:180px;vertical-align:top">' + label + '</td>' +
...
async function sendEmail(subject, htmlContent, toEmail) {
  ...
  if (!response.ok) throw new Error('Brevo API ' + response.status + ': ' + responseBody);
...
function buildCodeEmailHtml(name, code, minutes) {
  return '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;border:1px solid #e3e8ef;border-radius:10px;overflow:hidden">' +
    '<div style="background:#0b5cd8;color:#fff;padding:18px 24px"><h2 style="margin:0;font-size:20px">Your Security Code</h2></div>' +
...
app.post('/api/login', async function (req, res) {
  ...
  try {
    ...
    if (!user || !(await bcrypt.compare(String(password), user.password))) return res.status(401).json({ success: false, message: 'Invalid email or password.' });
...
app.post('/api/contact', requireAuthApi, async function (req, res) {
  ...
  const missing = required.filter(function (field) { return !String(data[field] || '').trim(); });
  if (missing.length) return res.status(400).json({ success: false, message: 'Missing fields: ' + missing.join(', ') });
  const booking = { name: data.name, phone: data.phone, email: data.email, serviceType: data.serviceType, date: data.date, time: data.time, address: data.address, message: data.message || '', submittedAt: new Date() };
  try {
    const db = await getDb();
